...
ImpGT <- read.table(paste0("Results_Impute5/",condition, "/", rep, "/Inversions/",inv,"/ImpGT"), sep = "\t", header = F)
...
ImpGT$Filt <- sapply(ImpGT$GP, function(x){
  max(as.numeric(str_split(string = x, pattern = ",", simplify = T)))
})
...
write.table(EUR_Exp, paste0("Results_Impute5/",condition, "/", rep,"/R2/", inv, "/", fChar, "/EUR_Exp.vcf"), sep = "\t", row.names = F, col.names = T,
            quote = F)
write.table(EUR_Imp, paste0("Results_Impute5/",condition, "/", rep,"/R2/", inv, "/", fChar, "/EUR_Imp.vcf"), sep = "\t", row.names = F, col.names = T,
            quote = F)
write.table(EAS_Exp, paste0("Results_Impute5/",condition, "/", rep,"/R2/", inv, "/", fChar, "/EAS_Exp.vcf"), sep = "\t", row.names = F, col.names = T,
            quote = F)
write.table(EAS_Imp, paste0("Results_Impute5/",condition, "/", rep,"/R2/", inv, "/", fChar, "/EAS_Imp.vcf"), sep = "\t", row.names = F, col.names = T,
            quote = F)
write.table(AFR_Exp, paste0("Results_Impute5/",condition, "/", rep,"/R2/", inv, "/", fChar, "/AFR_Exp.vcf"), sep = "\t", row.names = F, col.names = T,
            quote = F)
write.table(AFR_Imp, paste0("Results_Impute5/",condition, "/", rep,"/R2/", inv, "/", fChar, "/AFR_Imp.vcf"), sep = "\t", row.names = F, col.names = T,
            quote = F)
