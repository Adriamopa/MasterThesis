...
do
	...
	echo "$samples" | sed 's/\t/\n/g' > $wd/Results_Impute5/Inversions/$condition/$inv/indivs.txt
	grep -F -f $wd/Results_Impute5/Inversions/$condition/$inv/indivs.txt $wd/../VCFs/30X/Panel30x | cut -f1,4 > $wd/Results_Impute5/Inversions/$condition/$inv/SexedSamples
	...
	do
		...
		sex="$(grep "$samp" $wd/Results_Impute5/Inversions/$condition/$inv/SexedSamples | cut -f2)"
...
#      		sed -e 's/\t1\t/\t1|1\t/g' | sed -e 's/\t1$/\t1|1/g'  | sed -e 's/\t0$/\t0|0/g' | 
		bgzip > $wd/Results_Impute5/Inversions/$condition/$inv/Ref.vcf.gz
...
#      		sed 's/\t0\t/\t0|0\t/g' | sed -e 's/\t0\t/\t0|0\t/g' | sed -e 's/\t1\t/\t1|1\t/g' | \
#      		sed -e 's/\t1\t/\t1|1\t/g' | sed -e 's/\t1$/\t1|1/g'  | sed -e 's/\t0$/\t0|0/g' | 
		bgzip > $wd/Results_Impute5/Inversions/$condition/$inv/Target.vcf.gz
