...
> $wd/Results_Stats/Inversions_Freq_AFR.txt
> $wd/Results_Stats/Inversions_Freq_EUR.txt
> $wd/Results_Stats/Inversions_Freq_EAS.txt
...
do
	...
	do
		...
		then
			...
			echo -e "$inv\t$total_freq" >> $wd/Results_Stats/Inversions_Freq_$pop.txt
		elif [ $num -eq 1 ]
		then
			freq=($(echo "$state" | awk '{print $2}'))
			echo -e "$inv: $freq\n"
			echo -e "$inv\t$freq" >> $wd/Results_Stats/Inversions_Freq_$pop.txt
		else
